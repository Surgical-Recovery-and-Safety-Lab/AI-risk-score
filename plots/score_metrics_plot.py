...
from pyrisk.metrics.plots import plot_prediction_distribution, plot_reliability_diagrams
        ...
        for version in args.version_numbers:
            ...
            for key in pipeline.calibrator_probabilities[i]:
                ...
                tmp_metric_dict[key] = compute_score_metrics(
                    ["auroc", "ap", "log_loss"],
                    y_test[:, i],
                    pipeline.predictor_probabilities[i][key],
                )
                tmp_metric_dict_cal[key] = compute_score_metrics(
                    ["auroc", "ap", "log_loss"],
                    y_test[:, i],
                    pipeline.calibrator_probabilities[i][key],
                )
