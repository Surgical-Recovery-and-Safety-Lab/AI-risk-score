...
def plot_mean_PR_curve(
    ...
    **kwargs,
    ...
    """
    Plots the prc curve of each fold and the mean prc curve.
    ...
    nb_points : int, default: 500
        Number of points for the mean prc curve.
    **kwargs
    ...
    for i in range(n_it):
        ...
        for fold in model_metrics.keys():
            ...
            aucs.append(model_metrics[fold]["ap"][i])

        # Compute mean and std for all folds
        mean_recall, lower_recall, upper_recall = compute_CI(recalls)
        mean_ap, lower_ap, upper_ap = compute_CI(aucs)
        value = f" AP {mean_ap[0]:.2f} "
        ci = f"(95% CI, [{lower_ap[0]:.2f} - {upper_ap[0]:.2f}])"
    ...
    for model in models:
        for i in range(2):
            ...
            plot_mean_PR_curve(
                ...
                save_path=save_file + "_prc_" + model + ext[i],
                extension=extension,
                n_bins=20,
                dpi=300,
                figsize=(5, 5),
