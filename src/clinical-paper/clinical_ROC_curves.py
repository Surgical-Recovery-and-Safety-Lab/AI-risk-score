...
# -*- coding: utf-8 -*-
"""
clinical_ROC_curves.py

Plots ROC curves for the clinical paper.
"""
...
def plot_ROC_curve(
    ...
    **kwargs
    ...
    fpr, tpr, _ = roc_curve(y_test, proba_list)

    boots = []

    for _ in range(n_bootstraps):
        idx = np.random.choice(len(y_test), len(y_test), replace=True)
        fpr_boot, tpr_boot, _ = roc_curve(y_test[idx], proba_list[idx])
    ...
    try:
        ...
        for i, outcome in enumerate(pipeline.label_list):
            ...
            plot_ROC_curve(
                ...
                outcome=outcome,
                label="ROC",
                save_path=save_file + f"_{outcome}_ROC_curve",
                extension=extension,
