...
# -*- coding: utf-8 -*-
"""
clinical_site_calibration.py

Plots calibration curves per DHB.
"""
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Plot the calibration of each outcome per site"
    )
    parser.add_argument(
        "model_config_file",
    ...
    try:
        ...
        for dhb_name in X_test["DHB_NAME"].unique():
            for i, outcome in enumerate(pipeline.label_list):
                print_message(outcome, logger, script_name)
                idx = X_test["DHB_NAME"] == dhb_name
                y_pred_proba = pipeline.predict_proba(
                    X_test[idx], label_list=outcome, model_type=MODEL_MAP[outcome]
                )
                plot_clinical_calibration(
                    ...
                    label="Calibration",
                    distribution=True,
                    save_path=save_file + f"/{dhb_name}/{outcome}_reliability_diagram",
                    extension=extension,
